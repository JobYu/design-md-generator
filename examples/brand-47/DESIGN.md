...
### Shadows
- Brand uses **almost no shadows** in its dark theme. Depth is created through border contrast and surface color differences rather than box-shadows. Focus states use `rgba(0, 0, 0, 0.1) 0px 4px 12px` — minimal, functional.
...
*Note: Brand uses a notably minimal breakpoint system — primarily a single 600px breakpoint, suggesting a mobile-first approach with progressive enhancement.*
...
- **Border-defined space**: Instead of whitespace + shadows for separation, Brand uses thin borders on dark backgrounds — separation through line, not gap.
