...
- Brand Red (`#FF6363`) as a punctuation color — hero stripes, not pervasive
...
- **Brand Red** (`#FF6363` / `hsl(0, 100%, 69%)`): Brand accent — hero stripes, danger states, critical highlights

### Secondary & Accent
- **Brand Blue** (`hsl(202, 100%, 67%)` / ~`#55b3ff`): Interactive accent — links, focus states, selected items
- **Brand Green** (`hsl(151, 59%, 59%)` / ~`#5fc992`): Success states, positive indicators
- **Brand Yellow** (`hsl(43, 100%, 60%)` / ~`#ffbc33`): Warning accents, highlights
...
### Principles
- **Positive tracking on dark**: Unlike most dark UIs that use tight or neutral letter-spacing, Brand applies +0.2px to +0.4px — creating an airy, readable feel that compensates for the dark background
...
- **Display restraint**: Hero text at 64px/600 is confident but not oversized — Brand avoids typographic spectacle in favor of functional elegance
...
- **Transition**: All buttons use opacity transition for hover rather than background-color change — a signature Brand interaction pattern
...
- **Hero illustration**: Diagonal stripe pattern in Brand Red — abstract, geometric, brand-defining
- **App UI embeds**: Showing actual Brand command palette and extensions — product as content
...
### Shadow Philosophy
This product's shadow system is the most macOS-native on the web. Multi-layer shadows combine:
...
### Do
- Use `#07080a` (not pure black) as the background — the blue-cold tint is essential to the Brand feel
...
- Keep Brand Red (`#FF6363`) as punctuation, not pervasive — reserve it for hero moments and error states
...
### Don't
- Use pure black (`#000000`) as the background — the blue tint differentiates Brand from generic dark themes
- Apply negative letter-spacing on body text — Brand deliberately uses positive spacing for readability
- Use Brand Blue as the primary accent for everything — blue is for interactive/info, red is the brand color
...
- Brand Accent: Brand Red (`#FF6363`)
- Interactive Blue: Brand Blue (`hsl(202, 100%, 67%)` / ~`#55b3ff`)
...
- "Design an alert card with #101111 surface, Brand Red (#FF6363) left border accent, translucent red glow (hsla(0,100%,69%,0.15)), white heading, and #cecece description text"

### Iteration Guide
When refining existing screens generated with this design system:
1. Check the background is `#07080a` not pure black — the blue tint is critical
2. Verify letter-spacing is positive (+0.2px) on body text — negative spacing breaks the Brand aesthetic
