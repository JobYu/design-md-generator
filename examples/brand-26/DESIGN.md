...
- **No heavy shadows**: Brand relies on borders, not shadows, for depth.
...
- **Documentation-grade breathing room**: Every element has generous surrounding whitespace. Brand sells documentation, so the marketing page itself demonstrates reading comfort.
...
- **Content density is low**: Unlike developer tools that pack the page, Brand uses 1–2 key messages per section with supporting imagery.
...
### Iteration Guide
1. Always use full-pill radius (9999px) for buttons and inputs — this is This product's signature shape
