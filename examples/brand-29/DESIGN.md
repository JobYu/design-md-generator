...
- **Brand Green** (`#00ed64`): Primary brand accent — neon green for highlights, underlines, gradients
...
### Font Families
- **Display Serif**: `Brand Value Serif` — editorial hero headlines
...
|------|------|------|--------|-------------|----------------|-------|
| Display Hero | Brand Value Serif | 96px (6.00rem) | 400 | 1.20 (tight) | normal | Serif authority |
| Display Secondary | Brand Value Serif | 64px (4.00rem) | 400 | 1.00 (tight) | normal | Serif sub-hero |
...
### Principles
- **Serif for authority**: Brand Value Serif at hero scale creates an editorial presence unusual in tech — it communicates that Brand is an institution, not a startup.
...
- Background: `#00684a` (muted Brand green)
- Text: `#000000`
...
- Brand logo (leaf icon + wordmark) left-aligned
...
**Shadow Philosophy**: This product's shadow system is unique in that the primary elevation shadow uses `rgba(0, 30, 43, 0.12)` — a teal-tinted shadow that carries the forest-dark brand color into the depth system. This means even on white surfaces, shadows feel like they belong to the Brand color world rather than being generic neutral black.

## 7. Do's and Don'ts

### Do
- Use `#001e2b` (forest-black) for dark sections — not pure black
- Apply Brand Green (`#00ed64`) sparingly for maximum electric impact
- Use Brand Value Serif ONLY for hero/display headings — Euclid Circular A for everything else
...
- Don't use Brand Green (`#00ed64`) on backgrounds — it's an accent for text, underlines, and small highlights
- Don't use standard gray shadows — always use teal-tinted (`rgba(0,30,43,...)`)
- Don't apply serif font to body text — Brand Value Serif is hero-only
...
### Collapsing Strategy
- Hero: Brand Value Serif 96px → 64px → scales further
...
- Brand accent: Brand Green (`#00ed64`)
...
- "Create a hero on forest-black (#001e2b) background. Headline at 96px Brand Value Serif weight 400, line-height 1.20, white text with 'potential' highlighted in Brand Green (#00ed64). Subtitle at 18px Euclid Circular A weight 400. Green pill CTA (#00684a, 100px radius). Neon green gradient glow behind product screenshot."
...
- "Build a dark section: #001e2b background, 1px solid #3d4f58 border on cards. White text. Brand Green (#00ed64) accent underlines on headings using bottom-border 2px solid."
...
2. Brand Green (#00ed64) is electric — use once per section for maximum impact
3. Serif headlines (Brand Value Serif) create the editorial authority — never use for body
...
6. Teal-tinted shadows keep everything in the Brand color world
