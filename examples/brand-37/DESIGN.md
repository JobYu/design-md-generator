...
Typography is unified under NouvelR — a proprietary geometric sans-serif designed by Black[Foundry] exclusively for This product's rebrand. The typeface features a distinctive "radical r" with a terminal cut at 28 degrees to echo the Brand diamond logo's angles. Available in 6 weights from Light to Extrabold, the site primarily uses Bold (700) for headings and Regular (400) for body. Display headlines run large — 56px/0.95 line-height for hero titles, creating dense, impactful text blocks that sit tight against each other. The font supports Latin, Greek, Cyrillic, Hebrew, Arabic, and Korean, reflecting This product's global market reach. All text rendering feels precise and engineered, with the geometric proportions lending a sense of modernity that aligns with This product's electric-first brand positioning.
...
- Brand Yellow (`#EFDF00`) as the super-primary accent — used sparingly for highest-priority CTAs
...
- Link hover state in Brand Blue (`#1883FD`) — the sole chromatic interaction color

## 2. Color Palette & Roles

### Primary
- **Brand Yellow** (`#EFDF00`): The brand's signature Pantone — a vivid, saturated yellow used for super-primary CTAs and the highest-priority action buttons. Appears as `--CtaLink-background-color` on `.is-cta-super-primary` class. Carries the energy of the diamond logo
...
### Secondary & Accent
- **Soft Yellow** (`#F8EB4C`): Lighter, warmer variant of Brand Yellow — used for hover/pressed states on yellow CTAs and secondary accent contexts
- **Brand Blue** (`#1883FD`): Link hover color across all link variants — a bright, confident blue that signals interactivity without competing with the yellow brand accent
...
### Neutrals & Text
- **Absolute Black** (`#000000`): Primary heading and body text on light surfaces — Brand uses true black rather than near-black
...
### Font Family
- **NouvelR**: The sole typeface. A proprietary geometric sans-serif designed by Black[Foundry] for This product's 2021+ rebrand. Features a distinctive "radical r" with a 28-degree terminal cut matching the diamond logo angle. Available in 6 weights (Light to Extrabold), supports 6 writing systems. Fallback: `sans-serif`. Declared as `"NouvelR, sans-serif"` in CSS
- **No secondary typeface**: Unlike Ferrari (FerrariSans + Body-Font) or Lamborghini (LamboType + Open Sans), Brand uses a single font family for all text — headings, body, buttons, captions, and navigation
...
- **Bold-default headings**: Weight 700 dominates the heading hierarchy. Unlike brands that use medium (500) for headings, This product's Bold weight creates a more assertive, energetic reading experience
...
- **28-degree radical r**: The typeface's signature detail — the lowercase "r" terminal is cut at precisely 28 degrees to mirror the angles of the Brand diamond logo, embedding brand identity into every word
...
### Buttons
This product's buttons are sharp-edged rectangles with zero border-radius — the industrial precision of a pressed metal body panel.

**Super Primary (Yellow)** — The highest-emphasis CTA:
- Default: bg `#EFDF00` (Brand Yellow), text `#000000`, borderRadius 0px, padding 10px 15px, border 1px solid `#EFDF00`
...
- Hover: color shifts to `#1883FD` (Brand Blue), text-decoration none
...
### Navigation
- **Desktop**: Brand diamond logo centered/left, horizontal nav links, sticky positioning
- **Background**: white, no shadow at rest
- **Links**: NouvelR, 13px, weight 700, black text
- **Hover**: color shifts to `#1883FD` (Brand Blue)
...
### Do
- Use Brand Yellow (`#EFDF00`) exclusively for super-primary CTAs — it carries the full weight of the diamond logo's identity
- Maintain zero border-radius on all buttons — sharp edges are non-negotiable in the Brand system
...
- Use `#1883FD` (Brand Blue) consistently for all link hover states — one interactive color signal
...
### Don't
- Apply Brand Yellow as a background color for sections or surfaces — it's a CTA signal, not an atmosphere color
...
### Collapsing Strategy
- **Navigation**: Full horizontal nav collapses to Brand diamond logo + hamburger menu on mobile
...
- Primary CTA (Super): "Brand Yellow (#EFDF00)"
...
- "Create a dark E-Tech section on Absolute Black (#000000) with a NouvelR Bold 40px white heading 'E-Tech electric powertrain', a 14px subtitle in white, and a Brand Yellow (#EFDF00) super-primary button with black text, 0px radius, and 10px 15px padding"
...
1. Focus on ONE component at a time — This product's system has clear component boundaries (PromoCard, VehicleRangeCard, CTA variants)
...
6. Reserve Brand Yellow for ONE button per screen — if yellow appears in more than one CTA, the hierarchy collapses
