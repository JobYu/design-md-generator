...
- **Racing Yellow** (`#FFF200`): Heritage accent from This product's racing livery — reserved for special highlights and motorsport-related contexts (--f-color-yellow-hypersail)
...
### Buttons
This product's buttons are minimal white rectangles with near-zero radius — the CTA philosophy is "architecture, not decoration."
...
### Shadow Philosophy
This product's approach to elevation is nearly as flat as Tesla's, but with a different rationale. Where Tesla avoids shadows for minimalism, Brand avoids them because the editorial photography provides all the visual depth needed. The single shadow token (`rgb(153, 153, 153) 1px 1px 1px 0px`) is extremely subtle — a 1-pixel whisper used only in utilitarian contexts like consent dialogs. The site communicates hierarchy through three strategies:
...
1. Focus on ONE component at a time — This product's editorial rhythm means each section is a self-contained vignette
