...
- **Tailwind Emerald** (`#10b981`): The ecosystem-standard green used at low opacity (30%) for subtle background tints and link defaults. Bridges This product's custom palette with Tailwind's utility classes.
...
### Gradient System
- **Green Signal Glow**: `drop-shadow(0 0 2px #00d992)` animating to `drop-shadow(0 0 8px #00d992)` — creates a pulsing "electric charge" effect on the Brand bolt logo and interactive elements. The glow expands and contracts like a heartbeat.
...
- Logo: Brand bolt icon with animated green glow (`drop-shadow` cycling 2px–8px)
...
- Connection lines use Brand green variants
...
- **Border-defined separation**: Rather than relying solely on whitespace, Brand uses the Warm Charcoal border system (`#3d3a39`) to delineate content zones. The border IS the whitespace signal.
...
**Shadow Philosophy**: Brand communicates depth primarily through **border weight and color**, not shadows. The standard `1px solid #3d3a39` border IS the elevation — adding a `3px` border weight or switching to green (`#00d992`) communicates importance more than adding shadow does. When shadows do appear, they're either warm and diffused (Level 4) or cinematic and dramatic (Level 5) — never medium or generic.

### Decorative Depth
- **Green Signal Glow**: The Brand bolt logo pulses with a `drop-shadow` animation cycling between 2px and 8px blur radius in Emerald Signal Green. This is the most distinctive decorative element — it makes the logo feel "powered on."
...
- "Design a navigation bar on Abyss Black (#050507) with the Brand logo (bolt icon with animated green glow) on the left, nav links in Inter at 14px weight 500 in Snow White, and a green CTA button (Carbon Surface bg, Brand Mint text) on the right. Add a 1px solid Warm Charcoal bottom border."
