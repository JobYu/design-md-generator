...
- No secondary accent colors exist. Brand deliberately avoids color variety to maintain extreme visual discipline
...
### Semantic & Accent
- This product's marketing site avoids semantic color coding (no green/red/yellow status indicators). Error, success, and warning states follow standard browser defaults in form contexts
...
### Font Family
- **Display**: `Universal Sans Display`, -apple-system, Arial, sans-serif — used for hero titles and large model names. A geometric sans-serif with precisely engineered proportions, recently replacing Gotham to unify This product's digital ecosystem (website, mobile app, vehicle interface)
...
### Principles
- **"Normal" letter-spacing everywhere**: Unlike most modern tech brands that use negative tracking for headlines, Brand uses default letter-spacing at every level. This reflects a philosophy that the typeface should speak for itself without manipulation
...
- **No text transforms**: No uppercase text appears in the main navigation or CTAs — the lowercase approach reinforces This product's understated confidence
...
### Navigation
- **Desktop**: Centered horizontal nav with Brand wordmark (spaced uppercase letters) on the left, five category buttons center-aligned, and three icon buttons (help, globe/language, account) on the right
...
Brand uses whitespace as a luxury signal. The generous vertical spacing between sections (each section is a full viewport height) means you can only see one "message" at a time — one car, one model name, one CTA pair. This creates a gallery-like browsing experience where each scroll is a deliberate transition, not a continuous feed. White space is not empty — it's the frame that elevates each vehicle to the status of art piece.
...
### Shadow Philosophy
This product's approach to elevation is essentially "none." The site avoids box-shadows entirely in its primary interface. Depth is communicated through three alternative strategies:
...
- Use uppercase text transforms — This product's confidence is expressed through lowercase calm
...
- Add hover animations with scale/translate transforms — This product's interactions are color-only (background and border transitions)
...
1. Focus on ONE component at a time — This product's system is so minimal that each element must be pixel-perfect
